fun! jumpy#map(decl, stmt) abort
    ...
    if has('win32')
        let decl = escape(a:decl, '\')
        let stmt = escape(a:stmt, '\')
    else
        let decl = a:decl
        let stmt = a:stmt
    endif
	...
	if l:map[0] isnot# '' && l:map[1] isnot# '' && l:decl isnot# ''
		for l:mode in ['n', 'o', 'x']
			exe printf('%snoremap <buffer> <silent> %s :<C-u>call jumpy#jump("%s", "%s", "next")<CR>%s',
						\ l:mode, l:map[0], fnameescape(l:decl), l:mode, l:after)
			exe printf('%snoremap <buffer> <silent> %s :<C-u>call jumpy#jump("%s", "%s", "prev")<CR>%s',
						\ l:mode, l:map[1], fnameescape(l:decl), l:mode, l:after)
		endfor
	endif

	if l:map[2] isnot# '' && l:map[3] isnot# '' && l:stmt isnot# ''
		for l:mode in ['n', 'o', 'x']
			exe printf('%snoremap <buffer> <silent> %s :<C-u>call jumpy#jump("%s", "%s", "next")<CR>%s',
						\ l:mode, l:map[2], fnameescape(l:stmt), l:mode, l:after)
			exe printf('%snoremap <buffer> <silent> %s :<C-u>call jumpy#jump("%s", "%s", "prev")<CR>%s',
						\ l:mode, l:map[3], fnameescape(l:stmt), l:mode, l:after)
		endfor
	endif
endfun
